...
def summitBuffer(x,y, buff):
    arcpy.AddMessage(buff)
    arcpy.AddMessage(type(buff))
...
def checkNeighbors(testValue, point):
    global summitFound
    while neighbors.length() != 0 and not summitFound:
        ...
        if poppedValue <= testValue and not summitFound:
            ...
            # Summit fails against buffer
                ...
                adjValues(seperatePoint(poppedPoint, 0), seperatePoint(poppedPoint, 1))
                checkNeighbors(float(arcpy.GetCellValue_management(srcLocation, poppedPoint).getOutput(0)), poppedPoint)
...
srcLocation = arcpy.GetParameterAsText(0)
...
# Locations of the shape file to be tested
infc = arcpy.GetParameterAsText(1)

# Buffer for end testing
buffer = arcpy.GetParameter(4)


# Directory of the shape file to be created with the results
newFCDir = arcpy.GetParameterAsText(2)


# Name of the shape file to be created with the results
...
# Check that the DEM is projected
spatial_ref = arcpy.Describe(srcLocation).spatialReference
if spatial_ref.PCSName == ' ':
    sys.exit('Error: DEM must be projected!')
else:
    arcpy.AddMessage("Input DEM should be in an equi-distance projection")
...
# Get the spatial reference of the srcLocation
spatialRef = arcpy.Describe(srcLocation).spatialReference
...
for row in arcpy.da.SearchCursor(pointFC, fields, spatial_reference=spatial_ref):
    ...
    if row[1] in multi_points and feature == 'Summit':
        pointsToAdd.append({'NAME': pointName, 'FID': pointFID, 'STATE': pointState, 'COUNTY': pointCounty,
                            'FEATURE': feature, 'POINT': pointShape, 'DISTANCE': 0, 'ELEV':float(arcpy.GetCellValue_management(srcLocation, point).getOutput(0))})
        continue
    ...
    if feature == 'Summit':
        ...
        testValue = float(arcpy.GetCellValue_management(srcLocation, point).getOutput(0))

        # Get its 8 neighbors values
        adjValues(seperatePoint(point, 0), seperatePoint(point, 1))

        # Test for a summit
        checkNeighbors(testValue, point)
